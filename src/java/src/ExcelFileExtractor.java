...
class ExcelFileExtractor
{
	...
	String Manager;
	private
	XSSFWorkbook readMetadata;
	private
	static XSSFWorkbook workbook;

	public
	ExcelFileExtractor(String args) throws FileNotFoundException
	{
		input_document = new FileInputStream(new File(args));
	}

	public
	void MetadataExtractor() throws IOException
	{
	...
	public
	static void CellsRowsExtractor(String args) throws IOException
	{
		...
		for(int i = 0; i <= lastRow; i++)
		{
			try
			{
				System.out.println(spreadsheet.getRow(i)
								   .getCell(1)
								   .getRichStringCellValue()
								   .toString());
			}
